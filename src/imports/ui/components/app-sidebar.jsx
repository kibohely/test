...
import { TeamSwitcher } from "./team-switcher.jsx"; // Adjusted path
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
...
export function AppSidebar({ ...props }) { // Removed React.ComponentProps<typeof Sidebar> for simplicity
  return (
    <Sidebar {...props} className="dark !border-none"> {/* Assuming 'dark' class and other Tailwind classes are globally available */}
      <SidebarHeader>
                ...
                <SidebarMenuItem key={item.title}>
                  ...
                  >
                    <a href={item.url}>
                      {item.icon && (
                        <item.icon
                          className="text-sidebar-foreground/50 group-data-[active=true]/menu-button:text-primary" // Assuming 'text-primary' is globally defined
                          size={22}
                          aria-hidden="true"
                        />
                      )}
